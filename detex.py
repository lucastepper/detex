...
def get_text_enclosed(text: str, prefix: str, suffix: str, remove: bool = True, block: bool = False):
...
def delete_equations(text: str):
    ...
    return  re.sub(r"\\begin{equation\*?}(.*?)\\end{equation\*?}\n?", "", text, flags=re.DOTALL)


# def delete_equations(text: str):
#     """ Delete all equations in equation, array or align environment"""
#     l1 = len(text)
#     temp = re.sub(r"\\begin{equation\*", "", text, flags=re.DOTALL)
#     l2 = len(temp)
#     res = l1 - l2
#     # for name in ["equation, array, align"]:
#     #     to_find = r"\\begin{" + name +  r"\*?}.*\\end{" + name + r"\*?}"
#     #     text = re.sub(to_find, "", text, flags=re.DOTALL)
#     return text
...
def get_all_captions(text: str) -> list[str]:
    ...
    fig_tab_bodies = re.findall(r"\\begin{(?:figure|table)\*?}(.*?)\\end{(?:figure|table)\*?}", text, flags=re.DOTALL)
...
def main():
    text_out = ""
    file = "/home/greedisgod/latex/gaussian_process_optimization/main.tex"
    ...
    with open("/tmp/main.tex", "w", encoding="UTF-8") as fh:
        fh.write(text)
